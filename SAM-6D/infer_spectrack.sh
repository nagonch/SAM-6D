...
#   - cube and objects dataset types
#   - all reflectivities: 0.0, 0.5, 0.7, 1.0
#   - both gt and synth depth modes
...
REFERENCE_VIEWS_DIR="$DATASET_ROOT/reference_views"
...
# ─── Object mapping ────────────────────────────────────────────────────────────
# Maps sequence name → mesh folder name (under object_meshes/)
...
# Maps sequence name → reference_views prefix (reference_views/{prefix}_{reflectivity}/)
declare -A SEQ_TO_REFVIEW
SEQ_TO_REFVIEW["bleach0"]="bleach"
SEQ_TO_REFVIEW["bleach_hard_00_03_chaitanya"]="bleach"
SEQ_TO_REFVIEW["cracker_box_reorient"]="cracker_box"
SEQ_TO_REFVIEW["cracker_box_yalehand0"]="cracker_box"
SEQ_TO_REFVIEW["mustard0"]="mustard"
SEQ_TO_REFVIEW["mustard_easy_00_02"]="mustard"
SEQ_TO_REFVIEW["sugar_box1"]="sugar_box"
SEQ_TO_REFVIEW["sugar_box_yalehand0"]="sugar_box"
SEQ_TO_REFVIEW["tomato_soup_can_yalehand0"]="tomato_soup_can"
...
print_progress() {
    ...
    if [ "$total_done" -gt 0 ] && [ "$elapsed" -gt 0 ]; then
        local remaining_frames=$(( TOTAL_FRAMES - total_done ))
        local secs_per_frame=$(( elapsed / total_done ))
        local eta=$(( remaining_frames * secs_per_frame ))
...
echo ""
echo "──────────────────────────────────────────"
echo " STEP 1 / 3  Preparing templates"
echo "──────────────────────────────────────────"
python3 "$(pwd)/prepare_spectrack_templates.py"
    ...
    for DATASET_TYPE in cube objects; do
        for REFL in "${REFLECTIVITIES[@]}"; do
            ...
            for SEQ_PATH in "$SEQ_DIR"/*/; do
                ...
                # Determine mesh and template for this sequence
                if [ "$DATASET_TYPE" = "cube" ]; then
                    MESH_NAME="cube"
                    REFVIEW_PREFIX="cube"
                else
                    MESH_NAME="${SEQ_TO_MESH[$SEQ]:-}"
                    REFVIEW_PREFIX="${SEQ_TO_REFVIEW[$SEQ]:-}"
                    if [ -z "$MESH_NAME" ]; then
                ...
                TEMPLATE_DIR="$REFERENCE_VIEWS_DIR/${REFVIEW_PREFIX}_${REFL}"
...
echo ""
echo "──────────────────────────────────────────"
echo " STEP 3 / 3  Converting results to numpy"
echo "──────────────────────────────────────────"
