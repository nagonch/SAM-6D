...
def get_parser():
    ...
    parser.add_argument(
        "--seg_path",
        nargs="?",
        help="Path to segmentation information(generated by ISM)",
    )
    parser.add_argument(
        "--det_score_thresh", default=0.2, help="The score threshold of detection"
    )
...
def get_test_data(
    rgb_path, depth_path, cam_path, cad_path, seg_path, det_score_thresh, cfg
):
    dets = []
    ...
    whole_depth = (
        load_im(depth_path).astype(np.float32) * cam_info["depth_scale"] / 1000.0
    )
